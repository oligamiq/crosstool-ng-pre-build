...
struct sd_lun {
	...
	kstat_t	*un_pstats[NSDMAP];	/* partition statistics */
	kstat_t	*un_stats;		/* disk statistics */
	sd_unmapstats_t	*un_unmapstats;		/* UNMAP stats structure */
	kstat_t	*un_unmapstats_ks;	/* UNMAP kstat */
	kstat_t		*un_errstats;		/* for error statistics */
	kmutex_t	un_errstats_lock;		/* kstat lock */
	boolean_t	un_errstats_destroy;		/* destroy kstat */
...
#define	SD_FI_MAX_ERROR 1024
	...
	uint_t		un_soft_hba; /* hw or soft hba for io_time values */
	uint_t		un_iscsi_ssd; /* use iscsi devices as ssd */
};
...
/* Device error kstats */
struct sd_errstats {
	kstat_named_t	sd_softerrs;
	kstat_named_t	sd_harderrs;
	kstat_named_t	sd_transerrs;
	kstat_named_t	sd_vid;
	kstat_named_t	sd_pid;
	kstat_named_t	sd_revision;
	kstat_named_t	sd_serial;
	kstat_named_t	sd_capacity;
	kstat_named_t	sd_rq_media_err;
	kstat_named_t	sd_rq_ntrdy_err;
	kstat_named_t	sd_rq_nodev_err;
	kstat_named_t	sd_rq_recov_err;
	kstat_named_t	sd_rq_illrq_err;
	kstat_named_t	sd_rq_pfa_err;
	kstat_named_t	sd_ssd;
	kstat_named_t	sd_rotational;
	kstat_named_t	sd_devid;
	kstat_named_t	sd_enclosure;
	kstat_named_t	sd_slot;
	kstat_named_t	sd_hba;
	kstat_named_t	sd_scsi_wwn;
	kstat_named_t	sd_device_blksize;
	kstat_named_t	sd_device_sys_blksize;
	kstat_named_t	sd_inquiry_device_type;
	kstat_named_t	sd_soft_hba;
};
...
{									\
	kstat_t *pksp = (un)->un_pstats[SDPART((bp)->b_edev)];	\
	if (pksp != NULL) {						\
		kstat_io_t *kip = KSTAT_IO_PTR(pksp);			\
		size_t n_done = (bp)->b_bcount - (bp)->b_resid;		\
		if ((bp)->b_flags & B_READ) {				\
